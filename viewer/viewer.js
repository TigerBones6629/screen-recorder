...
function renderSteps(steps) {
  ...
  if (!steps.length) {
    stepsContainer.innerHTML =
      '<p class="empty">No steps captured yet. Turn on Step Guide Capture and start a session, then click through your workflow.</p>';
    return;
  }
  ...
  steps.forEach((step, index) => {
    ...
    captionInput.addEventListener('change', () => {
      window.stepViewer.updateCaption(index, captionInput.value);
      currentSteps[index].caption = captionInput.value;
    });
...
async function load() {
  const steps = await window.stepViewer.getSteps();
  renderSteps(steps || []);
}
...
document.getElementById('exportHtmlBtn').addEventListener('click', () => {
  const html = buildExportHtml(currentSteps);
